...
export class SelectSeatsTwoWayComponent implements OnInit {
    ...
    onChangeTime(time:any,type:number) {
        if(type==0){
            this.onGetSeatsStatus(this.busRouterGo.id, time.value, this.step1.daygo, type);
            this.listSeatsChooseGo = [];
            this.totalPriceGo = 0;
        }else{
            this.onGetSeatsStatus(this.busRouterReturn.id, time.value, this.step1.returnday, type);
    ...
    load() {
        ...
        this.onGetRouterIdById(this.step1.departure.id, this.step1.destination.id, date,0)
        this.onGetRouterIdById(this.step1.destination.id, this.step1.departure.id, date,1)
    }
    ...
    onGetRouterIdById(departureId: Int16Array, destinationId: Int16Array, date: string, type: number) {
        this.routerService
    ...
    onGetSeatsStatus(routerId: Int16Array, time: string, date: string, type: number){
       this.seatsService
       .getStatusSeat(routerId, time, date)
       .subscribe(
           data => {
